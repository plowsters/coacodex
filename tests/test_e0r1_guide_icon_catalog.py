...
def test_production_writer_renders_placeholders_for_a_referenced_client_catalog(tmp_path):
    """This test asserted that a `converted` row reached the renderer as a bundle asset. T2.5 prohibited
    that status and T6.3 removed the key it lived on, so the guarantee that remains — and the one that
    always mattered — is that NOTHING renders a remote image."""
    catalog = {sid: {"spell_id": sid, "asset_ref": f"{sid:032d}", "readiness": "available"}
               for sid in _NODE_IDS}
    out = write_report_outputs(_report(), tmp_path, formats=("html",), entries_path=ENTRIES,
                               icon_catalog=catalog)
    html = _html(out)
    assert "icons.tar#" not in html                     # no bundle asset exists to render
...
def test_load_client_icon_catalog_keys_by_spell_id(tmp_path):
    ...
    p.write_text('{"spell_id": 133, "asset_ref": "aa", "readiness": "available"}\n'
                 '{"spell_id": 116, "asset_ref": null, "readiness": "unavailable"}\n', encoding="utf-8")
    cat = load_client_icon_catalog(p)
    assert cat[133]["asset_ref"] == "aa" and cat[116]["asset_ref"] is None
