...
from coa_client_extract.contracts import (observation_state_code, observation_state_name,
                                          policy_ref)
from coa_client_extract.spell_record import build_field_descriptors
...
def test_iter_spell_records_is_a_generator_and_compact_raw():
    policy = v2_policy()
    gen = iter_spell_records(spell_dbc(), side_views(), policy=policy,
                             provenance={"effective_archive": "patch-T.MPQ"})
    assert isinstance(gen, types.GeneratorType)          # streaming, not a materialized list
    rows = list(gen)
    r = rows[0]
    assert r["schema_version"] == "coa-client-spell-v4" and r["spell_id"] == 133
    ...
    # E0R.2 T6.2: the pointer is hoisted into the descriptors, the vocabulary is interned.
    assert build_field_descriptors(policy.doc)["fields"]["power_type"]["policy_ref"] == \
        policy_ref("Spell", "power_type")
    assert r["raw"]["power_type"]["s"] == observation_state_code("present")
...
def test_raw_only_join_withholds_normalized_but_keeps_compact_raw():
    ...
    assert observation_state_name(r["raw"]["cast_time_ms"]["s"]) in ("resolved", "not_applicable")
